...
/**
 * App frontend testing with multiple languages not domained.
 */
class AppFrontendMultipleLanguagesTest extends \Tobento\App\Testing\TestCase
{
    ...
    public function testLocalizedLoginUrls()
    {
        ...
        $this->assertSame(
            [
                'default' => 'http://localhost/anmelden',
                'translated' => [
                    'de' => 'http://localhost/anmelden',
                    'fr' => 'http://localhost/fr/anmelden',
                ],
            ],
            $this->toUrls($url)
        );
    ...
    public function testLocalizedLoginRoutes()
    {
        ...
        $http->request(method: 'GET', uri: 'http://localhost/fr/anmelden');
        $http->response()->assertStatus(200)->assertBodyContains('Anmelden');
